...
\begin{titlepage}
    ...
    \begin{flushleft}
        ...
        \textbf{ID:} s345156 \> \textbf{ID:} s345701 \\
        \end{tabbing}
    \end{flushleft}
...
\end{tabular}
\end{table}
\subsection{Pro features}
The pro features we have implemented are:
\begin{itemize}
    \item extended instruction set (including division)
    \item branch prediction unit
    \item Hazard control with the use of stalls
\end{itemize}
\clearpage
...
The sparse tree makes use of two types of blocks, \textbf{PG} and \textbf{G}, whose outputs are computed recursively as:
\[
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\textwidth, keepaspectratio]{images/schematic.png}
  \caption{Top-level view of the synthesized architecture}
\end{figure}

The design was synthesized using a script developed for Design Vision.
The script analyzes all source files, elaborates a container module that gathers the components to be synthesized, and then generates the timing and power reports along with all files required by the subsequent CAD flow.
The container module includes the control unit and the datapath, but excludes both the instruction RAM and the data RAM.
The system clock period was set to 3 ns.
...
\begin{enumerate}
    \item \textbf{Floorplan definition:} Innovus divides the available area into the region allocated for standard cells and the region dedicated to power distribution.
    \item \textbf{Insertion of power rings and stripes:} these structures connect VDD and GND to all cells.
    \item \textbf{Cell placement:} the standard cells are positioned within the defined placement region while avoiding the layers reserved for power delivery.
    \item \textbf{Signal routing:} the interconnections between cells are created, primarily using the lower metal layers.
\end{enumerate}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.8\textwidth, keepaspectratio]{images/place_and_route.png}
  \caption{Connections in the final design}
\end{figure}

\subsection{Reports Analisys}

\begin{itemize}
\item The \textbf{Area report} (DLX\_NO\_RAM.gateCount) shows a relatively large total area of 59\,729.5\,\textmu m$^{2}$. 
      This is mainly due to a small number of modules that dominate the silicon footprint. 
      In particular, the \textbf{four largest contributors} are the following:

      \begin{itemize}
          \item \textbf{Branch Target Buffer} — by far the largest block, with an area of 31\,551.9\,\textmu m$^{2}$, representing more than half of the entire design.  
                Its size is due to the large number of entries required for branch prediction.
         
          \item \textbf{Register File} — the multi-ported register file requires 9\,051.4\,\textmu m$^{2}$, contributing significantly due to its storage density and multiple read/write ports.
          \item \textbf{Divider unit} — although included within the ALU hierarchy, it alone accounts for 3\,567.9\,\textmu m$^{2}$ and stands as one of the largest individual computational blocks.
          \item \textbf{Multiplier unit} — also within the ALU hierarchy, it requires 8\,214.1\,\textmu m$^{2}$ and is the second most area-expensive computational block after the BTB.
      \end{itemize}
      ...
      \item The \textbf{Power report} (power.txt) shows that with the chosen clock frequency of 333mhz and the 90nm technology used the total power consumed 
      amounts to 30.93 mW.

        The power is distributed as 19.17\,mW (62\%) internal, 10.58\,mW (34\%) switching, and 1.18\,mW (4\%) leakage.  
        Sequential logic dominates power usage (49\%), followed by combinational logic (37\%) and the clock tree (14\%).
       

      \item The \textbf{Timing report} (DLX\_NO\_RAM\_postRoute.slk) shows a positive slack for all possible paths, so no violations in these regards.
      More specifically the lowest slack is 0.593 and this suggests that the clock frequency has room for improvement.
\end{itemize}
